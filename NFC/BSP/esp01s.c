...
typedef struct {
    ...
    uint8_t               tcpConnWaitOk;   /**< TCP连接两阶段检测: 已收到CONNECT,等待OK */
} ESP01S_Instance_t;
...
static ESP01S_Config_t s_defaultConfig = {
    .ssid        = "Blitz X100s",           /**< WiFi热点名称 */
    .password    = "GUGUGAGA",        /**< WiFi密码 */
    .tcpServerIP = "",                 /**< TCP中继服务器IP(空=禁用,如"192.168.1.100") */
    .tcpPort     = 0,                  /**< TCP中继服务器端口(0=禁用,如8081) */
...
int ESP01S_Start(void)
{
    ...
    {
        printf("[ESP01S] WiFi connect failed!\r\n");
        return -2;
    }
...
void ESP01S_ExitTransparent(void)
{
    ...
    /* 后续静默+等待模块响应OK(回调中检测到OK会将state推进到TCP_CONNECTED) */
    ESP01S_DELAY(1500);

    /* 兜底: 若回调未收到OK(模块不在透传模式或+++时序不对),手动回退 */
...
void ESP01S_SyncNtpTime(void)
{
    ...
    {
        printf("[ESP01S] NTP OK(AT SNTP): %04d-%02d-%02d %02d:%02d:%02d\r\n",
     ...
     * 延迟500ms等待模块返回">"提示符(弱网或模块忙时200ms不够) */
    ...
    {
        printf("[ESP01S] NTP OK(UDP): %04d-%02d-%02d %02d:%02d:%02d\r\n",
               s_esp01s.ntpTime.year, s_esp01s.ntpTime.month, s_esp01s.ntpTime.day,
               s_esp01s.ntpTime.hour, s_esp01s.ntpTime.minute, s_esp01s.ntpTime.second);
    }
    else
    {
        printf("[ESP01S] NTP sync failed!\r\n");
    }
}
...
int ESP01S_SetRtcFromNtp(RTC_HandleTypeDef *pRtc)
{
    ...
    printf("[ESP01S] RTC calibrated: %04d-%02d-%02d %02d:%02d:%02d\r\n",
...
static int ESP01S_RestoreTcpTransparent(void)
{
    ...
    /* 前置清理: 确保无残留连接(如NTP UDP连接未完全关闭),为TCP连接腾出通道 */
    ESP01S_SendATCmd("AT+CIPCLOSE\r\n", 500);

    /* 连接配置的中继TCP服务器,超时增至5秒(弱网/DNS解析可能较慢) */
    printf("[ESP01S] TCP connecting to %s:%d ...\r\n",
           s_esp01s.config.tcpServerIP, s_esp01s.config.tcpPort);
    s_esp01s.state = ESP01S_STATE_TCP_CONNECTING;
    s_esp01s.tcpConnWaitOk = 0;
    sprintf(buf, "AT+CIPSTART=\"TCP\",\"%s\",%d\r\n", s_esp01s.config.tcpServerIP, s_esp01s.config.tcpPort);
    ESP01S_SendATCmd(buf, 5000);

    if (s_esp01s.state < ESP01S_STATE_TCP_CONNECTED)
    {
        printf("[ESP01S] TCP connect to %s:%d FAILED (state=%d)!\r\n",
               s_esp01s.config.tcpServerIP, s_esp01s.config.tcpPort, s_esp01s.state);
        return -3;
    }
    ...
    printf("[ESP01S] TCP transparent mode OK\r\n");
                        ...
                        char *outPrecip, uint16_t precipBufSize)
{
    ...
    if (s_esp01s.state < ESP01S_STATE_TCP_CONNECTED)
    {
        ...
        printf("[ESP01S] Weather DNS fail, try CIPDOMAIN...\r\n");
        ...
        if (s_esp01s.cipdomainResult[0] != '\0')
        {
            ...
            ESP01S_SendATCmd(cipBuf, 8000);
        }
        if (s_esp01s.state < ESP01S_STATE_TCP_CONNECTED)
        {
            printf("[ESP01S] Weather server connect failed!\r\n");
    ...
    /* 通过AT+CIPSEND发送HTTP请求,延迟500ms等待">"提示符 */
    snprintf(sendCmd, sizeof(sendCmd), "AT+CIPSEND=%d\r\n", reqLen);
    ESP01S_SendATCmd(sendCmd, 500);
    UartDrv_Send(s_esp01s.pUartDrv, (uint8_t *)req, reqLen);
    ESP01S_DELAY(500);

    /* 等待HTTP响应(心知天气响应通常在1~3秒内返回,弱网延长至8秒) */
    ESP01S_DELAY(4000);
    ...
    if (body != NULL)
    {
        ...
        /* 检测API错误响应: {"status":"...","status_code":"APxxxxxx"} */
        {
            ...
            if (ESP01S_JsonExtractString(body, "status_code", apiErr, sizeof(apiErr)) != NULL)
            {
                ...
                if (ESP01S_JsonExtractString(body, "status", apiMsg, sizeof(apiMsg)) == NULL)
                    apiMsg[0] = '\0';
                printf("[ESP01S] Weather API error: [%s] %s\r\n", apiErr, apiMsg);
                ret = -4;
                goto WEATHER_RESTORE;
            }
        }
        ...
        if (daily == NULL)
        {
            printf("[ESP01S] Weather JSON parse fail(no daily):\r\n%.120s\r\n", body);
            ret = -4;
            goto WEATHER_RESTORE;
        }
        ...
        {
            printf("[ESP01S] Weather JSON parse fail(daily field):\r\n%.120s\r\n", body);
            ret = -4;
            goto WEATHER_RESTORE;
        }
        ...
        printf("[ESP01S] Weather OK: %s day:%s %sC night:%s %sC rain:%s\r\n",
               outCity, outTextDay, outHigh, outTextNight, outLow, outPrecip);
        ret = 0;
    }
    else
    {
        printf("[ESP01S] Weather HTTP parse fail\r\n");
        ret = -4;
    }
...
/**
 * @brief  通过 wttr.in 查询天气, 响应为简单的管道分隔文本
 * @note   请求: GET /{location}?format="%l|%C|%t|%p" HTTP/1.1
 *         响应: "Hangzhou|Sunny|+28°C|15%"
 *         ESP8266 非透传模式下响应带 +IPD,<len>: 前缀, 需过滤
 */
int ESP01S_QueryWeatherWttr(const char *location,
                            char *outLine, uint16_t lineBufSize)
{
    char req[192];
    char sendCmd[32];
    uint16_t reqLen;
    ...
    if (location == NULL || outLine == NULL || lineBufSize == 0)
        return -1;
    ...
    /* 建立到 wttr.in 的 TCP 连接 (HTTP 80 端口) */
    printf("[ESP01S] Weather connecting to wttr.in:80 ...\r\n");
    s_esp01s.state = ESP01S_STATE_TCP_CONNECTING;
    s_esp01s.tcpConnWaitOk = 0;
    ESP01S_SendATCmd("AT+CIPSTART=\"TCP\",\"wttr.in\",80\r\n", 8000);
    printf("[ESP01S] Weather CIPSTART done, state=%d\r\n", s_esp01s.state);
    if (s_esp01s.state < ESP01S_STATE_TCP_CONNECTED)
    {
        printf("[ESP01S] wttr.in DNS fail, try CIPDOMAIN...\r\n");
        s_esp01s.cipdomainResult[0] = '\0';
        ESP01S_SendATCmd("AT+CIPDOMAIN=\"wttr.in\"\r\n", 3000);
        if (s_esp01s.cipdomainResult[0] != '\0')
        {
            char cipBuf[96];
            s_esp01s.state = ESP01S_STATE_TCP_CONNECTING;
        ...
        if (s_esp01s.state < ESP01S_STATE_TCP_CONNECTED)
        {
            printf("[ESP01S] wttr.in connect failed!\r\n");
            ret = -3;
            goto WTTR_RESTORE;
        }
    }

    /* 构造 HTTP GET 请求: 使用 wttr.in 格式说明符, %l=地点 %C=天气 %t=温度 %p=降水 */
    snprintf(req, sizeof(req),
             "GET /%s?format=%%l|%%C|%%t|%%p HTTP/1.1\r\n"
             "Host: wttr.in\r\n"
             "User-Agent: curl\r\n"
             "Connection: close\r\n"
             "\r\n",
    ...
    /* 设置天气查询等待标志, 回调将缓存原始响应数据 */
    s_esp01s.weatherPending = 1;
    s_esp01s.weatherRxLen = 0;
    s_esp01s.weatherRxBuf[0] = '\0';

    /* 发送HTTP请求,延迟500ms等待">"提示符 */
    snprintf(sendCmd, sizeof(sendCmd), "AT+CIPSEND=%d\r\n", reqLen);
    ESP01S_SendATCmd(sendCmd, 500);
    UartDrv_Send(s_esp01s.pUartDrv, (uint8_t *)req, reqLen);
    ...
    /* 调试: 打印原始响应 (前 200 字节) */
    printf("[ESP01S] wttr raw(%.200s)\r\n", s_esp01s.weatherRxBuf);
    ...
    if (body != NULL)
    {
        ...
        /* 过滤 ESP8266 非透传模式的 +IPD,<len>: 前缀 */
        char cleanBuf[256];
        uint16_t ci = 0;
        const char *p = body;
        while (*p && ci < sizeof(cleanBuf) - 1)
        {
            /* 跳过 +IPD,<len>: 标记 (如 "+IPD,48:"  "+IPD,1460:") */
            if (strncmp(p, "+IPD,", 5) == 0)
            {
                p += 5;
                while (*p >= '0' && *p <= '9') p++;  /* 跳过数字长度 */
                if (*p == ':') p++;                   /* 跳过 ':' */
                continue;
            }
            cleanBuf[ci++] = *p++;
        ...
        /* 去掉尾部换行 */
        uint16_t len = ci;
        while (len > 0 && (cleanBuf[len - 1] == '\r' || cleanBuf[len - 1] == '\n'))
            len--;
        cleanBuf[len] = '\0';

        if (len > 0 && len < lineBufSize)
        {
            memcpy(outLine, cleanBuf, len);
            outLine[len] = '\0';
            printf("[ESP01S] wttr.in OK: %s\r\n", outLine);
            ret = 0;
        }
        else
        {
            printf("[ESP01S] wttr.in body too long(%u) or empty\r\n", len);
            ret = -4;
        }
    }
    else
    {
        printf("[ESP01S] wttr.in HTTP parse fail (no \\r\\n\\r\\n found)\r\n");
        ret = -4;
    }
...
static void ESP01S_ProcessATResponse(const char *pstr, uint16_t len)
{
     ...
     * 当状态为TRANSPARENT时,收到OK说明+++退出透传成功。 */
    if (strstr(pstr, "OK") >= pstr)
    {
        ...
        else if (s_esp01s.state == ESP01S_STATE_TRANSPARENT)
        {
            /* +++退出透传成功,模块回复OK */
            s_esp01s.state = ESP01S_STATE_TCP_CONNECTED;
        }
     ...
     * 还需要等待最终的\r\nOK才算连接完成。
     * 使用 != NULL: 响应可能与前序命令的残留数据在同一帧中。 */
    if (strstr(pstr, "WIFI CONNECTED") != NULL)
    {
        s_esp01s.state = ESP01S_STATE_WIFI_CONNECTING;
    }
    ...
    /* ---- WiFi连接成功 ----
     * AT+CWJAP最终返回\r\nOK,表示连接过程完成。
     * CWJAP响应常为多行在同一帧: WIFI CONNECTED\r\nWIFI GOT IP\r\n\r\nOK
     * 因此\r\nOK不一定在行首,需用!=NULL查找。 */
    if (strstr(pstr, "\r\nOK") != NULL)
    {
     ...
     * UART空闲中断可能在"CONNECT\r\n"后触发第一帧,"\r\nOK\r\n"在第二帧,
     * 因此采用两阶段检测:
     *   阶段1: 捕获"CONNECT" → 设置tcpConnWaitOk=1
     *   阶段2: tcpConnWaitOk=1 且收到 "\r\nOK" → TCP_CONNECTED
     * 同时保留单帧完整匹配 "CONNECT\r\n\r\nOK" 的快速路径。
    ...
    if (s_esp01s.state == ESP01S_STATE_TCP_CONNECTING)
    {
        ...
        /* 阶段1: 捕获"CONNECT"(不含OK),等待后续OK帧 */
        else if (!s_esp01s.tcpConnWaitOk && strstr(pstr, "CONNECT") != NULL)
        {
            s_esp01s.tcpConnWaitOk = 1;
        }
        /* 阶段2: 已收到CONNECT,等待OK完成连接 */
        else if (s_esp01s.tcpConnWaitOk && strstr(pstr, "OK") != NULL)
        {
            s_esp01s.state = ESP01S_STATE_TCP_CONNECTED;
            s_esp01s.tcpConnWaitOk = 0;
        }
        /* ALREADY CONNECTED: 已连接 */
        if (strstr(pstr, "ALREADY CONNECTED") != NULL)
        {
            s_esp01s.state = ESP01S_STATE_TCP_CONNECTED;
            s_esp01s.tcpConnWaitOk = 0;
        }
     ...
     * 如果WiFi链路异常,返回:
     * "link is not valid"
     * 失败时重置两阶段检测标志,并将状态回退到WIFI_CONNECTED
     * (防止上层误判为已连接继而发送HTTP请求导致busy错误) */
    if (s_esp01s.state == ESP01S_STATE_TCP_CONNECTING &&
        (strstr(pstr, "DNS fail") != NULL || strstr(pstr, "DNS FAIL") != NULL ||
         strstr(pstr, "CONNECT FAIL") != NULL || strstr(pstr, "ERROR") != NULL ||
         strstr(pstr, "link is not valid") != NULL))
    {
        printf("[ESP01S] CIPSTART fail: %s\r\n", pstr);
        s_esp01s.tcpConnWaitOk = 0;  /* 重置,避免残留状态 */
        s_esp01s.state = ESP01S_STATE_WIFI_CONNECTED;  /* 回退,让上层检测到失败 */
    }
     ...
     * 使用 != NULL + strstr返回值定位 */
    if (strstr(pstr, "+CWSAP:") != NULL)
    {
        const char *pMatch = strstr(pstr, "+CWSAP:");
        const char *pStart = pMatch + 7;  /* 跳过"+CWSAP:" */
        while (*pStart == ' ')
            pStart++;                   /* 跳过空格 */
        if (*pStart == '"')
            pStart++;                   /* 跳过开引号 */
     ...
     * 使用 != NULL + strstr返回值定位 */
    if (strstr(pstr, "+CWJAP:") != NULL)
    {
        const char *pMatch = strstr(pstr, "+CWJAP:");
        const char *pStart = pMatch + 7;  /* 跳过"+CWJAP:" */
        if (*pStart == '"')
            pStart++;                   /* 跳过开引号 */
        const char *pEnd = strstr(pStart, "\"");
        if (pEnd != NULL && pEnd > pStart)
    ...
    if (pNtp != NULL)
    {
        ...
        {
            s_esp01s.ntpTimeValid = 0;
            printf("[ESP01S] NTP time parse fail\r\n");
        }
    }
}
