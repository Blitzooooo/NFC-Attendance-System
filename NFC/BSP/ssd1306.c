 ...
 * @details �?持SSD1306和SH1106两�?�OLED控制�?�?
 *          - �?动�?�测芯片类型（通过I2C读取验证�?
 *          - �?持硬件I2C和软件模拟I2C
 *          - 提供完整的图形绘制API
 * @author  HDU.ZYU
...
/** @brief X轴翻�?配置 (0=正常, 1=翻转) */
#define OLED_FLIP_X    1

/** @brief Y轴翻�?配置 (0=正常, 1=翻转) */
...
typedef enum {
    CHIP_UNKNOWN = 0,   /**< �?识别 */
    CHIP_SSD1306  = 1,  /**< SSD1306�?�? */
    CHIP_SH1106   = 2   /**< SH1106�?�? */
} OLED_ChipTypeDef;
...
/**
 * @brief   读取数据（硬件I2C版本�?
 * @details SH1106需要先发送控制字�?0x40，再读取数据
 ...
 * @brief   读取数据（软件I2C版本�?
 * @details I2C读取时序�?
 *          START �? 设�?�地址(�?) �? 控制字节(0x40) �? 
 *          RESTART �? 设�?�地址(�?) �? 读取数据 �? STOP
 * @return  读取到的数据字节
 */
 ...
 *          
 *          检测方法：
 *          1. 向显存写入测试数�?(0xA5, 0x5A)
 *          2. 尝试读取显存数据
 *          3. SH1106能�?�回写入的数�?（�??一�?字节是dummy byte�?
 *          4. SSD1306无法读取，返回不匹配的数�?
 *          
 * @return  1: SH1106�?�?
 * @return  0: SSD1306�?�?
 */
...
void SSD1306_UpdateScreen(void)
{
    ...
    /* I2C 防卡死: 仅在 HAL 状态异常时复位, 避免高频 DeInit/Init 导致花屏 */
...
/**
 * @brief   �?充整�?屏幕
 * @param   color: �?充�?�色 (GUI_COLOR_BLACK=黑色, GUI_COLOR_WHITE=白色)
 */
 ...
 * @param   r: 半径
 * @param   c: 边�?��?�色
 * @note    使用�?点圆算法(Midpoint Circle Algorithm)
 */
