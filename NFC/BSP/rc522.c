...
/** 连续通信失败计数器 (超过阈值自动硬件复位 RC522, 防止状态机卡死) */
static uint16_t s_commFailCount = 0;
#define RC522_COMM_FAIL_THRESHOLD   5     /**< 连续 5 次通信失败后自动恢复 */
                              ...
                              uint32_t *pOutLenBit)
{
    ...
    /* 连续通信失败计数: 成功清零, 超时/错误累计, 超过阈值则硬件复位 */
    if (status == RC522_OK) {
        s_commFailCount = 0;
    } else {
        s_commFailCount++;
        if (s_commFailCount >= RC522_COMM_FAIL_THRESHOLD) {
            s_commFailCount = 0;
            RC522_Reset();
            s_io->delay_ms(5);
